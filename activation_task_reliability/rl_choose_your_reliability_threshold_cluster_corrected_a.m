function rl_choose_your_reliability_threshold_cluster_corrected_a()
...
for ICC_map=1:numel(ICCs_maps)
    ...
    for ROI=1:numel(ROIs)
        ...
        if regexp(ICCs_maps{ICC_map},'all_sample')
            group='all_sample';
            meas=regexp(fin{2},'_all_sample_','split');
        else
            group='patients';
            meas=regexp(fin{2},'_patients_','split');
        end
        
        for threshold=1:numel(Thresholds)
            if exist(sprintf('cluster_correction_adjusted/%.2f/masks_cluster_correction_by_map_and_ROI/%s/%s/%s/cluster_correction_mask+orig.BRIK',Thresholds{threshold},ROIs{ROI},meas{1}, group))
                [mask]=BrikLoad(sprintf('cluster_correction_adjusted/%.2f/masks_cluster_correction_by_map_and_ROI/%s/%s/%s/cluster_correction_mask+orig.BRIK',Thresholds{threshold},ROIs{ROI},meas{1}, group));
                mask(find(mask))=1;
